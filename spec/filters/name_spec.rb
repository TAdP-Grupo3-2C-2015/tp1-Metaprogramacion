...
describe NameFilter do
  ...
  context '.call' do
    ...
    context 'origin is a class' do
      ...
      it 'all selectors' do
        expect(Aspects.new.has_name(/xy/).call(test_class).map{|wrapper| wrapper.method}).to contain_exactly(public_method_xyy, private_method_xyz)
      end
    ...
    context 'origin is an object' do
      ...
      it 'all selectors' do
        expect(Aspects.new.has_name(/xy/).call(test_instance).map{|wrapper| wrapper.method}).to contain_exactly(public_method_xyy, private_method_xyz, public_method_xyyy, private_method_xyyz)
      end
