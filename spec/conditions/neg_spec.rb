...
describe '.neg' do
  ...
  context 'when negating a single filter' do
    ...
    it 'retrieve negated name filter' do
      expect(neg(has_name(/.*one_param/)).match(test_class)).to contain_exactly(public_no_param, private_no_param, private_two_param)
    end

    it 'retrieve negated parameter filter' do
      expect(neg(is_public).match(test_class)).to contain_exactly(private_no_param, private_two_param)
    end
    ...
    it 'retrieve negated visibility filter (private)' do
      expect(neg(is_private).match(test_class)).to contain_exactly(public_no_param, public_one_param)
    end

    it 'retrieve no matches' do
      expect(neg(has_name(/param/)).match(test_class)).to be_empty
    end
  ...
  context 'when negating multiple filters' do
    ...
    it 'methods with negated named and params (both)' do
      expect(neg(has_name(/public/), has_parameters(0)).match(test_class)).to contain_exactly(private_two_param)
    end

    it 'methods with negated named and params (mandatory)' do
      expect(neg(has_name(/private/), has_parameters(1, mandatory)).match(test_class)).to contain_exactly(public_no_param)
    end

    it 'methods with negated param name and visible' do
      expect(neg(is_private, has_parameters(1, /x/)).match(test_class)).to contain_exactly(public_no_param)
    end

    it 'methods with double neg filter should cancel each other' do
      expect(neg(neg(has_parameters(1, /x/))).match(test_class)).to contain_exactly(public_one_param)
    end

    it 'retrieve no matches' do
      expect(neg(has_name(/param/), is_private).match(test_class)).to be_empty
    end
